...
const blackHoleShader: ShaderConfig = {
  name: 'Black Hole',
  description: 'Cosmic black hole with swirling accretion disk and event horizon effects',
  category: 'cosmic',
  tier: 'pro',
  vertexShader: `
  ...
  fragmentShader: `
    ...
    void main() {
        ...
        color -= 0.2 * vec3(0.0, 0.0, u_time * 0.5); // u_speed equivalent

        float angle = -log2(length(uv));
        color = rotateZ(color, angle);

        float frequency = 1.4; // u_frequency equivalent
        float distortion = 0.01 * 1.0; // u_distortion equivalent
        ...
        noiseColor *= 2.0 * 1.0; // u_intensity equivalent
        noiseColor -= 0.1;
        noiseColor *= 0.188 * 1.0; // u_glow equivalent
        noiseColor += vec3(uv.xy, 0.0);

        float noiseColorLength = length(noiseColor);
        noiseColorLength = 0.770 - noiseColorLength;
        noiseColorLength *= 4.2 * 1.0; // u_glow equivalent
        ...
        fac += 0.1;
        fac *= 3.0 * 1.0; // u_intensity equivalent
